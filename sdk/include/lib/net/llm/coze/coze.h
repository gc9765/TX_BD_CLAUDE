...
struct coze_chat_platform_cfg {
    ...
    char    *bot_id;                //必填
//  音频输入参数
    char    *input_audio_format;    //可选，输入音频的格式，支持 pcm、wav、ogg。默认为 wav。
...
//  音频输出参数    
    ...
    char    *output_audio_mp3_config_sample_rate; //输出 mp3 音频的采样率，默认是 44100。支持 32000、44100、48000。
    char    *output_audio_mp3_config_bit_rate;//输出 mp3 音频的码率，支持 8000~1600000。
...
//打断参数
    char    *turn_detection_type;   //可选，语音检测类型，用于控制语音交互的检测方式，枚举值：server_vad、client_interrupt。
...
//其他asr参数
    char    *asr_config_user_language;//可选，用户说话的语种，默认为 common。选项包括： common：大模型语音识别，可自动识别中英粤。
                                        ...
                                        //ru：小模型语音识别，俄语。
    char   *user_id;
    char   *update_url;                 //可选，上传文件到coze 服务器的url
    char   *file_id;                    //可选，上传文件后,server返回的file_id,用以多模态语音聊天。
};
